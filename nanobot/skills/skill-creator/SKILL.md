...
When creating a new skill from scratch, always run the `create_skill.py` script. The script conveniently generates a new template skill directory that automatically includes everything a skill requires, making the skill creation process much more efficient and reliable.

Usage:

```bash
scripts/create_skill.py <skill-name> --path <output-directory> [--resources scripts,references,assets] [--examples]
```

Examples:

```bash
scripts/create_skill.py my-skill --path skills/public
scripts/create_skill.py my-skill --path skills/public --resources scripts,references
scripts/create_skill.py my-skill --path skills/public --resources scripts --examples
```
